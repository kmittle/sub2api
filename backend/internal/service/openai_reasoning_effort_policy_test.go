...
func TestApplyOpenAIReasoningEffortPolicy(t *testing.T) {
	tests := []struct {
		...
		{name: "maps kimi native thinking", body: `{"thinking":{"type":"enabled","effort":"xhigh"}}`, mappings: []ReasoningEffortMapping{{From: "xhigh", To: "high"}}, path: "thinking.effort", want: "high", changed: true},
